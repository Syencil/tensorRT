...
private:
    ...
    std::vector<std::vector<float>> nms(std::vector<std::vector<float>> features, float thres) const;
...
public:
    ...
    //! \param trtParams
    //! \param yoloParams
    Yolo(common::InputParams inputParams, common::TrtParams trtParams, common::YoloParams yoloParams);
    ...
    //! \return [xmin, ymin, xmax, ymax]
    std::vector<std::vector<float>> predOneImage(const cv::Mat &image, float posThres=-1, float nmsThres=-1);
};
